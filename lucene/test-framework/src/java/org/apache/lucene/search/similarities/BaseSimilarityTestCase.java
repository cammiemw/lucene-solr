...
/**
 * Abstract class to do basic tests for a similarity. NOTE: This test focuses on
 * the similarity impl, nothing else. The [stretch] goal is for this test to be
 * so thorough in testing a new Similarity that if this test passes, then all
 * Lucene/Solr tests should also pass. Ie, if there is some bug in a given
 * Similarity that this test fails to catch then this test needs to be improved!
 */
public abstract class BaseSimilarityTestCase extends LuceneTestCase {
  ...
  /**
   * returns a random corpus that is at least possible given the norm value for a
   * single document.
   */
  static CollectionStatistics newCorpus(Random random, int norm) {
    ...
    if (norm == 0) {
      // norms are omitted, but there must have been at least one token to produce
      // that norm
      lowerBound = 1;
    } else {
  ...
  /**
   * Tests scoring across a bunch of random terms/corpora/frequencies for each
   * possible document length. It does the following checks:
   * <ul>
   * <li>scores are non-negative and finite.
   * <li>score matches the explanation exactly.
   * <li>internal explanations calculations are sane (e.g. sum of: and so on
   * actually compute sums)
   * <li>scores don't decrease as term frequencies increase: e.g. score(freq=N +
   * 1) &gt;= score(freq=N)
   * <li>scores don't decrease as documents get shorter, e.g. score(len=M) &gt;=
   * score(len=M+1)
   * <li>scores don't decrease as terms get rarer, e.g. score(term=N) &gt;=
   * score(term=N+1)
   * <li>scoring works for floating point frequencies (e.g. sloppy phrase and span
   * queries will work)
   * <li>scoring works for reasonably large 64-bit statistic values (e.g.
   * distributed search will work)
   * <li>scoring works for reasonably large boost values (0 .. Integer.MAX_VALUE,
   * e.g. query boosts will work)
   * <li>scoring works for parameters randomized within valid ranges (see
   * {@link #getSimilarity(Random)})
   * </ul>
   */
        ...
        for (int k = 1; k < 256; k++) {
          ...
          for (int l = 0; l < 10; l++) {
            ...
            if (term.totalTermFreq() == term.docFreq()) {
              ...
              // there is at least one other document, and those must have at least 1 instance
              // each.
              ...
              if (random.nextBoolean()) {
                ...
                // this avoids testing frequencies of 0 which seem wrong to allow (we should
                // enforce computeSlopFactor etc)
            ...
            // we just limit the test to "reasonable" boost values but don't enforce this
            // anywhere.
            // too big, and you are asking for overflow. that's hard for a sim to enforce
  ...
  /**
   * runs for a single test case, so that if you hit a test failure you can write
   * a reproducer just for that scenario
   */
  private static void doTestScoring(Similarity similarity, CollectionStatistics corpus, TermStatistics term,
      float boost, float freq, int norm) throws IOException {
    ...
    try {
      ...
      Explanation explanation = scorer.explain(Explanation.match(freq, "freq, occurrences of term within document"),
          norm);
      ...
      // check score(freq-1), given the same norm it should be <= score(freq) [scores
      // non-decreasing for more term occurrences]
      ...
      // check explanation matches
      Explanation prevExplanation = scorer
          .explain(Explanation.match(prevFreq, "freq, occurrences of term within document"), norm);
      ...
      // check score(norm-1), given the same freq it should be >= score(norm) [scores
      // non-decreasing as docs get shorter]
      if (norm > 1) {
        ...
        // check explanation matches
        Explanation prevNormExplanation = scorer
            .explain(Explanation.match(freq, "freq, occurrences of term within document"), norm - 1);
        ...
        if (prevNormScore < score) {
          ...
          System.out.println(explanation);
          fail("score(" + freq + "," + (norm - 1) + ")=" + prevNormScore + " < score(" + freq + "," + norm + ")="
              + score);
        }
      }

      // check score(term-1), given the same freq/norm it should be >= score(term)
      // [scores non-decreasing as terms get rarer]
      if (term.docFreq() > 1 && freq < term.totalTermFreq()) {
        ...
        // check explanation matches
        Explanation prevTermExplanation = prevTermScorer
            .explain(Explanation.match(freq, "freq, occurrences of term within document"), norm);
        ...
        if (prevTermScore < score) {
          ...
          System.out.println(explanation);
          fail("score(" + freq + "," + (prevTerm) + ")=" + prevTermScore + " < score(" + freq + "," + term + ")="
              + score);
        }
      }
