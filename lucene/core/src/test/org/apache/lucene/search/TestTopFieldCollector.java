...
public class TestTopFieldCollector extends LuceneTestCase {
  ...
  private TopFieldCollector doSearchWithThreshold(int numResults, int thresHold,
      Query q, Sort sort, IndexReader indexReader) throws IOException {
    IndexSearcher searcher = new IndexSearcher(indexReader);
    TopFieldCollector tdc = TopFieldCollector.create(sort, numResults,
        thresHold);
    searcher.search(q, tdc);
    return tdc;
  }
  
  private TopDocs doConcurrentSearchWithThreshold(int numResults, int threshold,
      Query q, Sort sort, IndexReader indexReader) throws IOException {
    ...
    try {
      ...
      CollectorManager<TopFieldCollector,TopFieldDocs> collectorManager = TopFieldCollector
          .createSharedManager(sort, numResults, null, threshold);
  ...
  public void testSharedHitcountCollector() throws Exception {
    ...
    for (int i = 0; i < sort.length; i++) {
      ...
      CollectorManager<TopFieldCollector,TopFieldDocs> tsdc = TopFieldCollector
          .createSharedManager(sort[i], 10, null, Integer.MAX_VALUE);
  ...
  public void testSortWithoutTotalHitTracking() throws Exception {
    ...
    for (int i = 0; i < 2; i++) {
      Query q = new MatchAllDocsQuery();
      // check that setting trackTotalHits to false does not throw an NPE
      // because
  ...
  public void testTotalHits() throws Exception {
    ...
    IndexWriter w = new IndexWriter(dir, newIndexWriterConfig()
        .setMergePolicy(NoMergePolicy.INSTANCE).setIndexSort(sort));
    Document doc = new Document();
    ...
    for (int totalHitsThreshold = 0; totalHitsThreshold < 20; ++totalHitsThreshold) {
      for (FieldDoc after : new FieldDoc[] {null,
          new FieldDoc(4, Float.NaN, new Object[] {2L})}) {
        TopFieldCollector collector = TopFieldCollector.create(sort, 2, after,
            totalHitsThreshold);
        ScoreAndDoc scorer = new ScoreAndDoc();
        
        LeafCollector leafCollector1 = collector
            .getLeafCollector(reader.leaves().get(0));
        ...
        LeafCollector leafCollector2 = collector
            .getLeafCollector(reader.leaves().get(1));
        ...
        TopDocs topDocs = collector.topDocs();
        assertEquals(new TotalHits(4, TotalHits.Relation.EQUAL_TO),
            topDocs.totalHits);
      }
    }
  ...
  public void testTotalHitsWithScore() throws Exception {
    ...
    for (int totalHitsThreshold = 0; totalHitsThreshold < 20; ++totalHitsThreshold) {
      Sort sort = new Sort(FIELD_SCORE,
          new SortField("foo", SortField.Type.LONG));
      TopFieldCollector collector = TopFieldCollector.create(sort, 2, null,
          totalHitsThreshold);
      ScoreAndDoc scorer = new ScoreAndDoc();
  ...
  public void testComputeScoresOnlyOnce() throws Exception {
    ...
    Query query = new BooleanQuery.Builder().add(foo, Occur.SHOULD)
        .add(bar, Occur.SHOULD).add(baz, Occur.SHOULD).build();
    final IndexSearcher searcher = new IndexSearcher(reader);
    for (Sort sort : new Sort[] {new Sort(FIELD_SCORE),
        new Sort(new SortField("f", SortField.Type.SCORE))}) {
      final TopFieldCollector topCollector = TopFieldCollector.create(sort,
          TestUtil.nextInt(random(), 1, 2), Integer.MAX_VALUE);
  ...
  public void testPopulateScores() throws IOException {
    ...
    for (String queryText : new String[] {"foo", "bar"}) {
      ...
      for (boolean reverse : new boolean[] {false, true}) {
        ...
        Sort sort = new Sort(
            new SortField("sort", SortField.Type.LONG, reverse));
        ...
        for (int i = 0; i < sortedByFieldClone.length; ++i) {
          ...
          assertSame(((FieldDoc) sortedByFieldClone[i]).fields,
              ((FieldDoc) sortedByField[i]).fields);
          assertEquals(sortedByFieldClone[i].score,
              sortedByDoc[Arrays.binarySearch(sortedByDoc,
                  sortedByFieldClone[i],
                  Comparator.comparingInt(sd -> sd.doc))].score,
              0f);
        }
      }
    }
  ...
  public void testConcurrentMinScore() throws Exception {
    ...
    CollectorManager<TopFieldCollector,TopFieldDocs> manager = TopFieldCollector
        .createSharedManager(sort, 2, null, 0);
    ...
    LeafCollector leafCollector = collector
        .getLeafCollector(reader.leaves().get(0));
    leafCollector.setScorer(scorer);
    LeafCollector leafCollector2 = collector2
        .getLeafCollector(reader.leaves().get(1));
    ...
    LeafCollector leafCollector3 = collector3
        .getLeafCollector(reader.leaves().get(2));
    ...
    TopFieldDocs topDocs = manager
        .reduce(Arrays.asList(collector, collector2, collector3));
    assertEquals(11, topDocs.totalHits.value);
    assertEquals(new TotalHits(11, TotalHits.Relation.GREATER_THAN_OR_EQUAL_TO),
        topDocs.totalHits);
  ...
  public void testRandomMinCompetitiveScore() throws Exception {
    ...
    w.close();
    Query[] queries = new Query[] {new TermQuery(new Term("f", "A")),
        new TermQuery(new Term("f", "B")), new TermQuery(new Term("f", "C")),
    ...
    for (Query query : queries) {
      Sort sort = new Sort(
          new SortField[] {SortField.FIELD_SCORE, SortField.FIELD_DOC});
      TopFieldCollector fieldCollector = doSearchWithThreshold(5, 0, query,
          sort, indexReader);
      TopDocs tdc = doConcurrentSearchWithThreshold(5, 0, query, sort,
          indexReader);
